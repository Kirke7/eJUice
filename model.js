...
export function solve(draft,ingredients,batch=draft.batch){
  ...
  for(const {row,i} of entries){if(row.mode==='nic'){if(i.strength<=0)throw Error('En automatisk nikotinbase skal indeholde nikotin.');shares+=row.amount;continue;}const p=row.mode==='weight'?row.amount/100:0,q=row.mode==='gml'?row.amount:0;F+=p;G+=q;H+=p/i.density;J+=q/i.density;N+=p*i.strength/i.density;K+=q*i.strength/i.density;}
  ...
  return {items,total,batch,strength,pg:items.reduce((s,i)=>s+i.ml*i.pg,0)/batch,vg:items.reduce((s,i)=>s+i.ml*i.vg,0)/batch,ethanol:items.reduce((s,i)=>s+i.ml*i.ethanol,0)/batch,warnings:Math.abs(strength-draft.target)>.005?['Den beregnede nikotinstyrke afviger fra ønsket. Tilføj en automatisk nikotinbase eller ret de faste doser.']:[]};
}
